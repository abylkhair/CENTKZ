...
var ImagesDialog = {
	...
	insert : function(text, im) {
window.parent.document.getElementById($("#field").val()).value=text;

top.closeim();

top.initim(im, ""+$("#div").val()+"");
	}
};
...
$(function(){
	...
	$('.imageBlock0').live('dblclick', function(){
		...
		if(e.attr('type') == 'files')
		{
			...
			var text = e.attr('linkto');
		//	text += e.attr('fname');
		//	text += '</a> ' + ' ('+filesize+') ';
	window.parent.document.getElementById('f3').value=text;
		window.parent.document.getElementById('f4').value=e.attr('fname');
	top.closeim();
return;
		var im=0;
		}
